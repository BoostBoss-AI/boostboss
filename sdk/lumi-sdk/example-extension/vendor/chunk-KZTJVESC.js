...
var Client = class {
  ...
  async fetchAd(o, sessionId) {
    const url = this.opts.apiBase.replace(/\/$/, "") + "/api/mcp";
    const args = {
      context_summary: (o.context || "").slice(0, 1e3),
...
var TypedEmitter = class {
  handlers = {};
  on(event, handler) {
    if (!this.handlers[event]) this.handlers[event] = /* @__PURE__ */ new Set();
...
function injectStyles(target = document) {
  ...
  if (target.getElementById("lumi-styles")) {
    cssInjected = true;
    return;
  }
...
function buildCta(ad, format, onClick) {
...
function renderAd(el, ad, format, onClick) {
  injectStyles();
...
function renderBanner(el, ad, onClick) {
...
function renderSidebar(el, ad, onClick) {
...
function renderInline(el, ad, onClick) {
...
function renderInterstitial(el, ad, onClick) {
...
function fireImpressionBeacon(url) {
  try {
...
function unmountSlot(el, backdrop) {
...
// src/lumi.ts
var VERSION = "0.1.0";
var DEFAULT_API_BASE = "https://boostboss.ai";
...
var Lumi = class {
  ...
  async refresh(target) {
    ...
    if (target !== void 0) {
      ...
      const existing = this.slots.get(el);
      if (!existing) return;
      await this.render(el, { format: existing.format });
      return;
    }
    const entries = [...this.slots.entries()];
    for (const [el, state] of entries) {
      await this.render(el, { format: state.format });
    }
  }
